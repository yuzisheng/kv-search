...
object DistanceUtils {
   ...
   * euclidean distance
   */
  def euclideanDistance(s1: Seq[Double], s2: Seq[Double]): Double = {
    assert(s1.length == s2.length)
    sqrt(s1.zip(s2).map(p => pow(p._1 - p._2, 2)).sum)
  }

  /**
   * manhattan distance
   */
  def manhattanDistance(s1: Seq[Double], s2: Seq[Double]): Double = {
    assert(s1.length == s2.length)
   ...
   */
  def longestCommonSubSeq(s1: Seq[Double], s2: Seq[Double], threshold: Double): Int = {
    assert(s1.length == s2.length)
