  ...
  def queryOp(method: String, qs: Seq[(Int, Double)], k: Int, otherParam: String): (Seq[(Int, Double)], Double, Int) = {
    method match {
      ...
      case "kv-scan" =>
        ...
        hbaseScanAll(hbaseTableName, qs.map(_._2), k)
      case _ =>
    ...
    val diffKRes = ks.par.map(k => {
      val sampleRes = sampleQs.par.map(qs =>
        (
          ...
          queryOp("kv-scan", qs, k, hbaseTableName)._2
        )
      )
      ...
      val kvSanAvgQueryTime = sampleRes.map(_._4).sum / sampleRes.length
      (k, bfAvgQueryTime, our1AvgQueryTime, our2AvgQueryTime, kvSanAvgQueryTime)
    }).toList.sortBy(_._1)

    for (r <- diffKRes) {
      val (k, bfAvgQueryTime, our1AvgQueryTime, our2AvgQueryTime, kvSanAvgQueryTime) = r
      println(
        s"""
           |+++++++
           ...
           |kv-scan avg query time:                  [${kvSanAvgQueryTime}s];
  ...
  def sparkBlockFilterOp(hdfsPath: String, sampleBlockHdfsPath: String,
    ...
    val res = sampleQs.par.map(
      qs => {
        ...
        (sparkBlockFilter(data, k, delta, qs, isBlockFilter = true)._2,
          sparkBlockFilter(data, k, delta, qs, isBlockFilter = false)._2)
      }
    )
