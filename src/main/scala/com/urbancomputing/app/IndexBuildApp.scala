...
import com.urbancomputing.util.{HBaseUtils, RowKeyUtils}
...
object IndexBuildApp {
  ...
  def buildId2SeqIndex(rdd: RDD[String], hbaseTableName: String): Long = {
    ...
    HBaseUtils.createTableIfNotExists(hbaseTableName, Seq("default"))
    val jobConf = HBaseUtils.jobConfig(hbaseTableName)
  ...
  def buildTimeMaxMinId2BlockSeqIndex(rdd: RDD[String], timeBlockLen: Int, valueBlockLen: Int, hbaseTableName: String): Long = {
    ...
    val putRdd = convertRdd.map(record => {
      ...
      val rowKey = RowKeyUtils.timeMaxMinIdIndex(time, timeBlockSeqMax, timeBlockSeqMin, id)
      val put = new Put(rowKey)
